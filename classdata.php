...
$result = mysqli_query($conn, "SELECT nisn, kelas, tb_kelas.kelas_id FROM tb_kelas INNER JOIN tb_isi_kelas on tb_kelas.kelas_id = tb_isi_kelas.kelas_id WHERE nisn = '$nisn'");
if ($row2 = mysqli_fetch_assoc($result)) {
    $kelas = $row2["kelas"];
    $kelas_id = $row2["kelas_id"];
}

$classdata = query("SELECT emailski, tb_isi_kelas.nisn, nama_siswa, gender_siswa FROM tb_siswa INNER JOIN tb_isi_kelas on tb_siswa.nisn = tb_isi_kelas.nisn WHERE kelas_id=$kelas_id;");
    ...
    <div class="right">
        <div class="content">
            <div class="content-hello" style="position: absolute;">
                <div>
                    <h2 style="font-weight: 500;">Class Data</h2>
                </div>
                <div>
                ...
                <div>
                    <table style="width:1000px; margin-top:40px; ">
                        <thead>
                            <tr>
                                <th>Email SKI</th>
                                <th>NIS</th>
                        ...
                        <?php foreach ($classdata  as $key => $row) : ?>
                            <tb>
                                <tr>
